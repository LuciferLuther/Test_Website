...
export function ItineraryBuilder() {
  ...
  const copyPlan = async () => {
    const text = [
      ...
      ...stops.map(
        (stop, index) =>
          `${index + 1}. ${stop.label}: ${stop.nights} ${stop.nights === 1 ? "night" : "nights"} (${formatShortDate(stop.checkIn)}–${formatShortDate(stop.checkOut)})`,
      ),
      "",
  ...
  return (
    <section className="section section--paper plan-section" id="plan">
      <div className="container">
        <SectionHeading
          title="Exact itinerary."
          copy="Four hotel stays, sixteen nights, and one planned holiday move."
        />
        <div className="plan-layout">
          <Reveal
            className="plan-result aether-surface aether-surface--paper"
            data-aether="selected-itinerary"
          >
            <div className="plan-result__header">
              <div>
                <h3>{preset.name}</h3>
                <p>
                  {formatLongDate(travelStart)} — {formatLongDate(tripEnd)}
                </p>
              </div>
              <button
            ...
            <ol className="plan-timeline">
              {stops.map((stop, index) => {
                ...
                const nextCity = stops[index + 1]
                  ? cities.find((item) => item.id === stops[index + 1].cityId)
                  : undefined;
                const travelIcon =
                ...
                return (
                  <li key={`${stop.cityId}-${index}`}>
                    ...
                    <div>
                      ...
                      <h4>{stop.label}</h4>
                      <span>
                        {stop.nights} {stop.nights === 1 ? "night" : "nights"} · {stop.note}
                      </span>
                    </div>
            ...
            <div className="holiday-checks">
              <article>
                ...
                <p>Fly to Sapporo, then stop.</p>
              </article>
              <article>
                <span>1 Jan</span>
                <strong>New Year&apos;s Day</strong>
                <p>Stay in Sapporo with no hotel move.</p>
              </article>
            </div>
            <div className="route-status route-status--warn">
              <Icon name="sparkle" />
              <div>
                <strong>One planned holiday move</strong>
                <p>
                  {warnings[0]} Leave Hakone early, use a direct HND–CTS flight, and keep the
                  Sapporo evening simple.
                </p>
              </div>
            </div>
          </Reveal>
